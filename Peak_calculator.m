fds = fileDatastore('data/*.csv', 'ReadFcn', @importdata);
...
for k = 1 : numFiles
    ...
    %Reading data
    parx = readmatrix(fullFileNames{k}, 'range', 'A2:A201');
    pary = readmatrix(fullFileNames{k}, 'range', 'B2:D201');
    perpx = parx;
    perpy = readmatrix(fullFileNames{k}, 'range', 'D2:F201');

    %Adjusting baseline for parallel line scans
    YA = msbackadj(parx, pary);

    %Calculating PFWHH parallel
    [parpeaks, PFWHH] = mspeaks(parx, YA, 'Multiplier', 0.5);
    ...
    %Calculating mean peaks in parallel line scans (adjusted to /um)
    Ny = ((perpnumpeaks1+perpnumpeaks2+perpnumpeaks3)/3)/300;
   
    % Add the Nx value to the Nx_values array
    Nx_values(k) = Nx;
    Ny_values(k) = Ny;
    Dv_values(k) = ((Nx*Ny)/(1.5.*cos(45)))*1000000;
end
